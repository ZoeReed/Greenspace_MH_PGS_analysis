######################################
##Script to test association between depression PGS (at multiple thresholds) and NDVI
...
#Load PGS data
polygenic_risk_score_5e8<-read.csv("/filepath/grs_dep_5e8.csv")
polygenic_risk_score_000001<-read.csv("/filepath/grs_dep_000001.csv")
polygenic_risk_score_00001<-read.csv("/filepath/grs_dep_00001.csv")
polygenic_risk_score_0001<-read.csv("/filepath/grs_dep_0001.csv")
polygenic_risk_score_001<-read.csv("/filepath/grs_dep_001.csv")
polygenic_risk_score_01<-read.csv("/filepath/grs_dep_01.csv")
polygenic_risk_score_05<-read.csv("/filepath/grs_dep_05.csv")
polygenic_risk_score_1<-read.csv("/filepath/grs_dep_1.csv")
polygenic_risk_score_2<-read.csv("/filepath/grs_dep_2.csv")
polygenic_risk_score_3<-read.csv("/filepath/grs_dep_3.csv")
polygenic_risk_score_4<-read.csv("/filepath/grs_dep_4.csv")
polygenic_risk_score_5<-read.csv("/filepath/grs_dep_5.csv")
...
#Check variance explained in outcome by PCs
pcs<-list(green_data2$PC1, green_data2$PC2, green_data2$PC3, green_data2$PC4, green_data2$PC5, green_data2$PC6, green_data2$PC7, green_data2$PC8, green_data2$PC9, green_data2$PC10, green_data2$PC11, green_data2$PC12, green_data2$PC13, green_data2$PC14, green_data2$PC15, green_data2$PC16, green_data2$PC17, green_data2$PC18, green_data2$PC19, green_data2$PC20, green_data2$PC21, green_data2$PC22, green_data2$PC23, green_data2$PC24, green_data2$PC25)

names(pcs)<-c("PC1", "PC2", "PC3", "PC4", "PC5", "PC6", "PC7", "PC8", "PC9", "PC10", "PC11", "PC12", "PC13", "PC14", "PC15", "PC16", "PC17", "PC18", "PC19", "PC20", "PC21", "PC22", "PC23", "PC24", "PC25")
...
for(pc in pcs){
	i<-i+1
	pc_mod<-lm(green_data2$NDVI_500m_mean ~ pc)	
	rsq<-summary(pc_mod)$r.squared
	new<-as.data.frame(rsq)
	new$variable<-names(pcs[i])
	new$p<-summary(pc_mod)$coefficients[2,4]
	pc_mod_results<-rbind(pc_mod_results, new)
}

rm(new)

##Greenness~Dep PGS
Greenness_Dep_PRS_results<-list()
i<-0
...
for(risk_score in scores){
	...
	new$N<-nobs(model)
	Greenness_Dep_PRS_results<-rbind(Greenness_Dep_PRS_results, new)
}

write.table(Greenness_Dep_PRS_results, file="/filepath/Greenness_Dep_PRS_results.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)


#MSOA with random intercept
Greenness_Dep_PRS_msoa_results<-list()
i<-0
...
for(risk_score in scores){
	...
	Greenness_Dep_PRS_msoa_results<-rbind(Greenness_Dep_PRS_msoa_results, new)
}

write.table(Greenness_Dep_PRS_msoa_results, file="/filepath/Greenness_Dep_PRS_MSOA_results.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)

#VCP for MSOA
Greenness_Dep_PRS_msoa_vpc<-list()
i<-0
...
for(risk_score in scores){
	...
	new$score<-names(scores[i])
	Greenness_Dep_PRS_msoa_vpc<-rbind(Greenness_Dep_PRS_msoa_vpc, new)
}

write.table(Greenness_Dep_PRS_msoa_vpc, file="/filepath/Greenness_Dep_PRS_MSOA_VPC.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)

#LAD with random intercept
Greenness_Dep_PRS_lad_results<-list()
i<-0
...
for(risk_score in scores){
	...
	coefs<-data.frame(coef(summary(model)))
	new$p<-2*(1-pnorm(abs(coefs$t.value[2])))
	Greenness_Dep_PRS_lad_results<-rbind(Greenness_Dep_PRS_lad_results, new)
}

write.table(Greenness_Dep_PRS_lad_results, file="/filepath/Greenness_Dep_PRS_LAD_results.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)

#VCP for LAD
Greenness_Dep_PRS_lad_vpc<-list()
i<-0
...
for(risk_score in scores){
	...
	new<-as.data.frame(vpc$icc[1])
	new$score<-names(scores[i])
	Greenness_Dep_PRS_lad_vpc<-rbind(Greenness_Dep_PRS_lad_vpc, new)
}

write.table(Greenness_Dep_PRS_lad_vpc, file="/filepath/Greenness_Dep_PRS_LAD_VPC.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)
