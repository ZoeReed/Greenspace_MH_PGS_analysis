######################################
##Script to test association between wellbeing PGS (at multiple thresholds) and NDVI
...
#Load PGS data
polygenic_risk_score_5e8<-read.csv("/filepath/grs_well_5e8.csv")
polygenic_risk_score_000001<-read.csv("/filepath/grs_well_000001.csv")
polygenic_risk_score_00001<-read.csv("/filepath/grs_well_00001.csv")
polygenic_risk_score_0001<-read.csv("/filepath/grs_well_0001.csv")
polygenic_risk_score_001<-read.csv("/filepath/grs_well_001.csv")
polygenic_risk_score_01<-read.csv("/filepath/grs_well_01.csv")
polygenic_risk_score_05<-read.csv("/filepath/grs_well_05.csv")
polygenic_risk_score_1<-read.csv("/filepath/grs_well_1.csv")
polygenic_risk_score_2<-read.csv("/filepath/grs_well_2.csv")
polygenic_risk_score_3<-read.csv("/filepath/grs_well_3.csv")
polygenic_risk_score_4<-read.csv("/filepath/grs_well_4.csv")
polygenic_risk_score_5<-read.csv("/filepath/grs_well_5.csv")
...
green_data2<-green_data2[!(is.na(green_data2$NDVI_500m_mean)), ]
dim(green_data2) #N=238,306 participants

#Standardise variable
green_data2$NDVI_500m_mean<-scale(green_data2$NDVI_500m_mean, center=T, scale=T)
...
#removed lowest (GW) as not avialble
scores<-list(polygenic_risk_score_000001, polygenic_risk_score_00001, polygenic_risk_score_0001, polygenic_risk_score_001, polygenic_risk_score_01, polygenic_risk_score_05, polygenic_risk_score_1, polygenic_risk_score_2, polygenic_risk_score_3, polygenic_risk_score_4, polygenic_risk_score_5)

names(scores)<-c("polygenic_score_000001", "polygenic_score_00001", "polygenic_score_0001", "polygenic_score_001", "polygenic_score_01", "polygenic_score_05", "polygenic_score_1", "polygenic_score_2", "polygenic_score_3", "polygenic_score_4", "polygenic_score_5")

######################################
#Analysis
######################################

##Greenness~Wellbeing PRS
Greenness_Well_PRS_results<-list()
i<-0

for(risk_score in scores){
	i<-i+1
	model<-lm(green_data2$NDVI_500m_mean ~ risk_score$z_risk_score + green_data2$sex + green_data2$age + green_data2$PC1 + green_data2$PC2 + green_data2$PC3 + green_data2$PC4 + green_data2$PC5 + green_data2$PC6 + green_data2$PC7 + green_data2$PC8 + green_data2$PC9 + green_data2$PC10 + green_data2$PC11 + green_data2$PC12 + green_data2$PC13 + green_data2$PC14 + green_data2$PC15 + green_data2$PC16 + green_data2$PC17 + green_data2$PC18 + green_data2$PC19 + green_data2$PC20 + green_data2$PC21 + green_data2$PC22 + green_data2$PC23 + green_data2$PC24 + green_data2$PC25)
	modelb<-lm(green_data2$NDVI_500m_mean ~ risk_score$z_risk_score)
	...
	new$N<-nobs(model)
	Greenness_Well_PRS_results<-rbind(Greenness_Well_PRS_results, new)
}

write.table(Greenness_Well_PRS_results, file="/filepath/Greenness_Well_PRS_results.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)

#MSOA with random intercept
Greenness_Well_PRS_msoa_results<-list()
i<-0

for(risk_score in scores){
	i<-i+1
	model<-lmer(green_data2$NDVI_500m_mean ~ risk_score$z_risk_score + green_data2$sex + green_data2$age + green_data2$PC1 + green_data2$PC2 + green_data2$PC3 + green_data2$PC4 + green_data2$PC5 + green_data2$PC6 + green_data2$PC7 + green_data2$PC8 + green_data2$PC9 + green_data2$PC10 + green_data2$PC11 + green_data2$PC12 + green_data2$PC13 + green_data2$PC14 + green_data2$PC15 + green_data2$PC16 + green_data2$PC17 + green_data2$PC18 + green_data2$PC19 + green_data2$PC20 + green_data2$PC21 + green_data2$PC22 + green_data2$PC23 + green_data2$PC24 + green_data2$PC25 + (1|green_data2$census_msoa))
	N<-nobs(model)
	new<-as.data.frame(N)
	new$score<-names(scores[i])
	new$beta<-summary(model)$coefficients[2,1]
	...
	Greenness_Well_PRS_msoa_results<-rbind(Greenness_Well_PRS_msoa_results, new)
}

write.table(Greenness_Well_PRS_msoa_results, file="/filepath/Greenness_Well_PRS_MSOA_results.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)


#VCP for MSOA
Greenness_Well_PRS_msoa_vpc<-list()
i<-0
...
for(risk_score in scores){
	...
	new$score<-names(scores[i])
	Greenness_Well_PRS_msoa_vpc<-rbind(Greenness_Well_PRS_msoa_vpc, new)
}

write.table(Greenness_Well_PRS_msoa_vpc, file="/filepath/Greenness_Well_PRS_MSOA_VPC.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)


#LAD with random intercept
Greenness_Well_PRS_lad_results<-list()
i<-0
...
for(risk_score in scores){
	...
	coefs<-data.frame(coef(summary(model)))
	new$p<-2*(1-pnorm(abs(coefs$t.value[2])))
	Greenness_Well_PRS_lad_results<-rbind(Greenness_Well_PRS_lad_results, new)
}

write.table(Greenness_Well_PRS_lad_results, file="/filepath/Greenness_Well_PRS_LAD_results.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)

#VCP for LAD
Greenness_Well_PRS_lad_vpc<-list()
i<-0
...
for(risk_score in scores){
	...
	new<-as.data.frame(vpc$icc[1])
	new$score<-names(scores[i])
	Greenness_Well_PRS_lad_vpc<-rbind(Greenness_Well_PRS_lad_vpc, new)
}

write.table(Greenness_Well_PRS_lad_vpc, file="/filepath/Greenness_Well_PRS_LAD_VPC.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)
