...
##Greenspace~Wellbeing PRS
Greenspace_Well_PRS_results<-list()
i<-0
...
for(risk_score in scores){
	...
	new$N<-nobs(model)
	Greenspace_Well_PRS_results<-rbind(Greenspace_Well_PRS_results, new)
}

write.table(Greenspace_Well_PRS_results, file="/filepath/Greenspace_Well_PRS_results.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)

#MSOA with random intercept
Greenspace_Well_PRS_msoa_results<-list()
i<-0
...
for(risk_score in scores){
	...
	Greenspace_Well_PRS_msoa_results<-rbind(Greenspace_Well_PRS_msoa_results, new)
}

write.table(Greenspace_Well_PRS_msoa_results, file="/filepath/Greenspace_Well_PRS_MSOA_results.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)


#VCP for MSOA
Greenspace_Well_PRS_msoa_vpc<-list()
i<-0
...
for(risk_score in scores){
	...
	new$score<-names(scores[i])
	Greenspace_Well_PRS_msoa_vpc<-rbind(Greenspace_Well_PRS_msoa_vpc, new)
}

write.table(Greenspace_Well_PRS_msoa_vpc, file="/filepath/Greenspace_Well_PRS_MSOA_VPC.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)

#LAD with random intercept
Greenspace_Well_PRS_lad_results<-list()
i<-0
...
for(risk_score in scores){
	...
	coefs<-data.frame(coef(summary(model)))
	new$p<-2*(1-pnorm(abs(coefs$t.value[2])))
	Greenspace_Well_PRS_lad_results<-rbind(Greenspace_Well_PRS_lad_results, new)
}

write.table(Greenspace_Well_PRS_lad_results, file="/filepath/Greenspace_Well_PRS_LAD_results.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)

#VCP for LAD
Greenspace_Well_PRS_lad_vpc<-list()
i<-0
...
for(risk_score in scores){
	...
	new<-as.data.frame(vpc$icc[1])
	new$score<-names(scores[i])
	Greenspace_Well_PRS_lad_vpc<-rbind(Greenspace_Well_PRS_lad_vpc, new)
}

write.table(Greenspace_Well_PRS_lad_vpc, file="/filepath/Greenspace_Well_PRS_LAD_VPC.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)
