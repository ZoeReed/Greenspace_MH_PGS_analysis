######################################
##Script to test the association between the Wellbeing PRS (at multiple thresholds) and the NDVI variable
...
#Load PRS data
polygenic_risk_score_000001<-read.csv("/path/grs_well_000001.csv")
polygenic_risk_score_00001<-read.csv("/path/grs_well_00001.csv")
polygenic_risk_score_0001<-read.csv("/path/grs_well_0001.csv")
polygenic_risk_score_001<-read.csv("/path/grs_well_001.csv")
polygenic_risk_score_01<-read.csv("/path/grs_well_01.csv")
polygenic_risk_score_05<-read.csv("/path/grs_well_05.csv")
polygenic_risk_score_1<-read.csv("/path/grs_well_1.csv")
polygenic_risk_score_2<-read.csv("/path/grs_well_2.csv")
polygenic_risk_score_3<-read.csv("/path/grs_well_3.csv")
polygenic_risk_score_4<-read.csv("/path/grs_well_4.csv")
polygenic_risk_score_5<-read.csv("/path/grs_well_5.csv")
...
#Removed lowest (GW) as not avialable
...
Greenness_Well_PRS_lm_results<-list()
i<-0
...
for(risk_score in scores){
	...
	new$N<-nobs(summ_lm)
	Greenness_Well_PRS_lm_results<-rbind(Greenness_Well_PRS_lm_results, new)
	}

write.csv(Greenness_Well_PRS_lm_results, file="/path/Greenness_Well_PRS_results_lm.csv", row.names=F, quote=F)
...
Greenness_Well_PRS_msoa_results_mundlak_risk_score<-list()
Greenness_Well_PRS_msoa_results_mundlak_risk_score_mean<-list()
i<-0
...
for(risk_score in scores){
	...
	greenness_agg<-tmp_data %>% group_by(census_msoa) %>% 
	summarise_at(vars(sex, age, NDVI_500m_mean, z_risk_score, PC1, PC2, PC3, PC4, PC5,PC6, PC7, PC8, PC9, PC10, PC11, PC12, PC13, PC14, PC15, PC16, PC17, PC18, PC19, PC20, PC21, PC22, PC23, PC24, PC25), list(mean=mean))
	## Merge scz_prs_05_mean variable into broader UKB data for mundlak formulation
	greenness_edited <- left_join(tmp_data, greenness_agg[c("census_msoa","z_risk_score_mean")])
	## List variables for model specs
	outcome <- "NDVI_500m_mean"
	exposure <- "z_risk_score"
	...
	Greenness_Well_PRS_msoa_results_mundlak_risk_score<-rbind(Greenness_Well_PRS_msoa_results_mundlak_risk_score, new)
	Greenness_Well_PRS_msoa_results_mundlak_risk_score_mean<-rbind(Greenness_Well_PRS_msoa_results_mundlak_risk_score_mean, new2)
}

write.csv(Greenness_Well_PRS_msoa_results_mundlak_risk_score, file="/path/Greenness_Well_PRS_MSOA_results_Mundlak_risk_score.csv", row.names=F, quote=F)

write.csv(Greenness_Well_PRS_msoa_results_mundlak_risk_score_mean, file="/path/Greenness_Well_PRS_MSOA_results_Mundlak_risk_score_mean.csv", row.names=F, quote=F)
