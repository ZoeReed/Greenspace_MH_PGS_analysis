######################################
##Script to test the association between the Depression PRS (at multiple thresholds) and the Greenspace variable
...
Greenspace_Dep_PRS_lm_results<-list()
i<-0
...
for(risk_score in scores){
	...
	new$N<-nobs(summ_lm)
	Greenspace_Dep_PRS_lm_results<-rbind(Greenspace_Dep_PRS_lm_results, new)
}

write.csv(Greenspace_Dep_PRS_lm_results, file="/path/Greenspace_Dep_PRS_results_lm.csv", row.names=F, quote=F)
...
Greenspace_Dep_PRS_msoa_results_mundlak_risk_score<-list()
Greenspace_Dep_PRS_msoa_results_mundlak_risk_score_mean<-list()
i<-0
...
for(risk_score in scores){
	...
	greenspace_agg<-tmp_data %>% group_by(census_msoa) %>% 
	summarise_at(vars(sex, age, greenspace_percent_300m, z_risk_score, PC1, PC2, PC3, PC4, PC5,PC6, PC7, PC8, PC9, PC10, PC11, PC12, PC13, PC14, PC15, PC16, PC17, PC18, PC19, PC20, PC21, PC22, PC23, PC24, PC25), list(mean=mean))
	## Merge Dep_prs_05_mean variable into broader UKB data for mundlak formulation
	greenspace_edited <- left_join(tmp_data, greenspace_agg[c("census_msoa","z_risk_score_mean")])
	## List variables for model specs
	outcome <- "greenspace_percent_300m"
	exposure <- "z_risk_score"
	...
	Greenspace_Dep_PRS_msoa_results_mundlak_risk_score<-rbind(Greenspace_Dep_PRS_msoa_results_mundlak_risk_score, new)
	Greenspace_Dep_PRS_msoa_results_mundlak_risk_score_mean<-rbind(Greenspace_Dep_PRS_msoa_results_mundlak_risk_score_mean, new2)
}

write.csv(Greenspace_Dep_PRS_msoa_results_mundlak_risk_score, file="/path/Greenspace_Dep_PRS_MSOA_results_Mundlak_risk_score.csv", row.names=F, quote=F)

write.csv(Greenspace_Dep_PRS_msoa_results_mundlak_risk_score_mean, file="/path/Greenspace_Dep_PRS_MSOA_results_Mundlak_risk_score_mean.csv", row.names=F, quote=F)
