######################################
##Script to test association between schizophrenia PGS (at multiple thresholds) and NDVI
...
dim(green_data2)
#after all of those exclusions the data has 238,306 participants
...
##Greenness~Scz PRS
Greenness_Scz_PRS_results<-list()
i<-0
...
for(risk_score in scores){
	...
	new$N<-nobs(model)
	Greenness_Scz_PRS_results<-rbind(Greenness_Scz_PRS_results, new)
}

write.table(Greenness_Scz_PRS_results, file="/filepath/Greenness_Scz_PRS_results.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)

#MSOA with random intercept
Greenness_Scz_PRS_msoa_results<-list()
i<-0
...
for(risk_score in scores){
	...
	Greenness_Scz_PRS_msoa_results<-rbind(Greenness_Scz_PRS_msoa_results, new)
}

write.table(Greenness_Scz_PRS_msoa_results, file="/filepath/Greenness_Scz_PRS_MSOA_results.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)

#VCP for MSOA
Greenness_Scz_PRS_msoa_vpc<-list()
i<-0
...
for(risk_score in scores){
	...
	new$score<-names(scores[i])
	Greenness_Scz_PRS_msoa_vpc<-rbind(Greenness_Scz_PRS_msoa_vpc, new)
}

write.table(Greenness_Scz_PRS_msoa_vpc, file="/filepath/Greenness_Scz_PRS_MSOA_VPC.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)

#LAD with random intercept
Greenness_Scz_PRS_lad_results<-list()
i<-0
...
for(risk_score in scores){
	...
	coefs<-data.frame(coef(summary(model)))
	new$p<-2*(1-pnorm(abs(coefs$t.value[2])))
	Greenness_Scz_PRS_lad_results<-rbind(Greenness_Scz_PRS_lad_results, new)
}

write.table(Greenness_Scz_PRS_lad_results, file="/filepath/Greenness_Scz_PRS_LAD_results.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)


#VCP for LAD
Greenness_Scz_PRS_lad_vpc<-list()
i<-0
...
for(risk_score in scores){
	...
	new<-as.data.frame(vpc$icc[1])
	new$score<-names(scores[i])
	Greenness_Scz_PRS_lad_vpc<-rbind(Greenness_Scz_PRS_lad_vpc, new)
}

write.table(Greenness_Scz_PRS_lad_vpc, file="/filepath/Greenness_Scz_PRS_LAD_VPC.csv", row.names=F, append=F, col.names=T, sep="\t", quote=F)
