...
#PGS
scz_grs<-read.csv(file="/filepath/grs_scz_5e8.csv")
...
all<-merge(phen, scz_grs, by.x="IID", by.y="id") 

all$sex<-as.factor(all$sex)
all$schizophrenia_diagnosis<-as.factor(all$schizophrenia_diagnosis)
all$schizophrenia_non_cancer<-as.factor(all$schizophrenia_non_cancer)
all$schizophrenia_icd_primary<-as.factor(all$schizophrenia_icd_primary)
all$schizophrenia_icd_secondary<-as.factor(all$schizophrenia_icd_secondary)
all$Scz_MH_online<-as.factor(all$Scz_MH_online)
...
table(all$schizophrenia_diagnosis)
sum(!is.na(all$schizophrenia_diagnosis))

#Schizophrenia MH online
table(all$Scz_MH_online)

#Schizophrenia ICD primary and secondary
table(all$schizophrenia_icd_primary)
table(all$schizophrenia_icd_secondary)
table(all$schizophrenia_icd_primary, all$schizophrenia_icd_secondary) 

#Schizophrenia non cancer codes
table(all$schizophrenia_non_cancer)
...
#Percentage greenspace and schizophrenia
summary(lm(all$greenspace_percent_300m ~ all$schizophrenia_diagnosis)) 
nobs(lm(all$greenspace_percent_300m ~ all$schizophrenia_diagnosis)) #293,922

summary(lm(all$greenspace_percent_300m ~ all$schizophrenia_diagnosis + all$age + all$sex))
nobs(lm(all$greenspace_percent_300m ~ all$schizophrenia_diagnosis + all$age + all$sex)) #293,922

#Schizophrenia and confounders (age, sex)
#Age (schizophrenia diagnois)
model1<-lm(all$age~all$schizophrenia_diagnosis)
summary(model1)

#Sex (schizophrenia diagnois)
chi_mod<-chisq.test(all$schizophrenia_diagnosis,all$sex)
chi_mod$observed
...
tsls<-ivreg(greenspace_percent_300m ~ schizophrenia_diagnosis + age + sex + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PC11 + PC12 + PC13 + PC14 + PC15 + PC16 + PC17 + PC18 + PC19 + PC20 + PC21 + PC22 + PC23 + PC24 + PC25 | grs + age + sex + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PC11 + PC12 + PC13 + PC14 + PC15 + PC16 + PC17 + PC18 + PC19 + PC20 + PC21 + PC22 + PC23 + PC24 + PC25, data=all)
