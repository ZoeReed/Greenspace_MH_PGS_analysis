...
#PGS
scz_grs<-read.csv("/filepath/grs_scz_05.csv")
...
mod<-glm(all$schizophrenia_diagnosis ~ all$grs, family="binomial")
...
mod<-lm(all$NDVI_500m_mean ~ all$schizophrenia_diagnosis + all2$sex + all2$age)
summary(mod)
nobs(mod)

#Association between schizophrenia diagnosis and percentage greenspace, adjusted for sex and age
mod<-lm(all$greenspace_percent_300m ~ all$schizophrenia_diagnosis + all2$sex + all2$age)
summary(mod)
nobs(mod)
