...
#PGS
dep_grs<-read.csv("/filepath/grs_dep_05.csv")
...
mod<-glm(all$depression_diagnosis ~ all$grs, family="binomial")
...
mod<-lm(all$NDVI_500m_mean ~ all$depression_diagnosis + all2$sex + all2$age)
summary(mod)
nobs(mod)

#Association between MHQ dep and NDVI, adjusted for sex and age
mod<-lm(all$NDVI_500m_mean ~ all$MHQ_dep + all2$sex + all2$age)
summary(mod)
nobs(mod)

#Association between depression diagnosis and percentage greenspace, adjusted for sex and age
mod<-lm(all$greenspace_percent_300m ~ all$depression_diagnosis + all2$sex + all2$age)
summary(mod)
nobs(mod)

#Association between MHQ dep and percentage greenspace, adjusted for sex and age
mod<-lm(all$greenspace_percent_300m ~ all$MHQ_dep + all2$sex + all2$age)
summary(mod)
nobs(mod)
