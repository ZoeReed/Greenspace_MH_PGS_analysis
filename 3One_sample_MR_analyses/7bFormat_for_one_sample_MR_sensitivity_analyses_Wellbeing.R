######################################
##Script to format data for one sample MR sensitivity analyses for wellbeing
...
#Genotype data
Wellbeing_gen<-read.table("/filepath/Wellbeing_snps.raw", header=T)
...
all_data<-all_data[!is.na(all_data$rs3756290_G),]
...
write.csv(data, "/filepath/Percentage_GS_data_wellbeing_adjusted_summary_data_ukb.csv", row.names = F)
