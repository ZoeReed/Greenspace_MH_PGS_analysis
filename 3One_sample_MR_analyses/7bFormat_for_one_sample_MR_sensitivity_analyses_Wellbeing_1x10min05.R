######################################
##Script to format data for one sample MR sensitivity analyses for wellbeing 1x10-05
...
#Genotype data
Wellbeing_gen<-read.table("/filepath/Wellbeing_1x10min05_snps.raw", header=T)
...
all_data<-merge(phen, Wellbeing_gen, by="IID", all.x=T)
...
all_data<-all_data[!is.na(all_data$rs710212_C),]
...
#Percentage greenspace only (Wellbeing as exposure)
#Summary statistics of genetic associations of Wellbeing and outcome
...
X.1 <- as.matrix(as.numeric(all_data[,6])) #Exposure X.1 is wellbeing 
...
write.csv(data, "/filepath/Percentage_GS_data_wellbeing_1x10min05_adjusted_summary_data_ukb.csv", row.names = F)
