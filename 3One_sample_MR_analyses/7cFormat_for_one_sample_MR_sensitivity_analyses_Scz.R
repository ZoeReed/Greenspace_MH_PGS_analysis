...
#Genotype data
Scz_gen<-read.table("/filepath/Scz_snps.raw", header=T)

#Phenotypic
load("/filepath/phenotypic_one_sample_MR.RData")

#GWAS
Scz_gwas<-read.table("/filepath/exposure_dat_harmonised_scz_5e8.csv", header=T, sep=",")

######################################
#Format data
######################################

#Flip alleles as needed so it matches GWAS effect allele
Scz_gwas_subset<-Scz_gwas[,c("SNP", "effect_allele.exposure")]

#Inspect manually
colnames(Scz_gen)
Scz_gwas_subset

#These was one SNP that needed flipping: rs9274390 change T to A
Scz_gen$rs9274390_A<-2-Scz_gen$rs9274390_T
Scz_gen$rs9274390_T<-NULL
...
all_data<-merge(phen, Scz_gen, by="IID", all.x=T)
...
all_data<-all_data[!is.na(all_data$rs4648845_T),]
...
#Percentage greenspace only (Scz diag as exposure)
#Summary statistics of genetic associations of Schizophrenia and outcome
...
X.1 <- as.matrix(as.numeric(all_data[,9])) #Exposure X.1 is scz diag 
...
write.csv(data, "/filepath/Percentage_GS_data_scz_diag_adjusted_summary_data_ukb.csv", row.names = F)
